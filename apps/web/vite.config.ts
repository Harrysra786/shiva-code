...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__DSH_BOOT__. '
  ...
  + 'For client-plugin HMR, run `pnpm dsh web` together with a watcher that rewrites client bundles.'
const DEFAULT_CLIENT_TITLE = 'ShivaCode'
...
function clientDocumentTitle(): Plugin {
  ...
  return {
    ...
    transformIndexHtml(html) {
      return html.replace('<title>ShivaCode</title>', `<title>${title}</title>`)
    },
  }
}
