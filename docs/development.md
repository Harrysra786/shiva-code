...
- `pre-push` runs `pnpm run typecheck`, which completes the Host lib phase, including generated Typert contracts, before the Client TypeScript check. It also compares `desktop/package-lock.json` against the plugin tarballs it names, but only when the push carries the lockfile or one of those tarballs: `npm pack` embeds an mtime, so repacking rewrites every tarball, and a partial `npm install` leaves the rest with a stale hash that `npm install` reconciles silently and `npm ci` refuses on the release runner.
