...
开发先使用 `pnpm run build` 准备当前 Host、Client 与 Web 产物，然后运行源码 Host：

```sh
pnpm dsh web
```

`dsh` 通过 tsx 启动 Host 源码，所以 Host 可以使用 SRC 回退。本仓库不再提供 Client plugin watcher：改动 Client 插件后用 `pnpm run build` 重新构建，它会重写该插件的 `lib/client.js`，宿主的 stat 轮询随即广播重载。
