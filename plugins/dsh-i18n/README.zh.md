...
已附带的 locale 服务（`@deepseek-ai/dsh-client-locale`）拥有偏好、选择器以及 ns×locale 字典注册表。本插件只向它贡献内容：`addLanguage` 把语言加入可选集合（以 `en` 作为回退），每个 namespace 一次 `register(namespace, locale, dictionary)` 调用提供其文案。已附带的内容不会被替换——移除插件后，面板与此前完全一致，只是少了这两项。
...
导航栏中的每个标签都在本插件所供给的 locale 注册表之外解析。`dsh-better-sidebar` 自带 zh／en 字典，并用 `startsWith('zh')` 判断在两者间选择，因此其他任何语言都会落到英文；其他插件注册的 tab 要么是同样的双语处理，要么是纯字面量。任何字典都无法触及它们——没有可供覆盖的键。

侧边栏在**渲染时调用** tab 的标题，这留下了唯一一处诚实的接缝。本插件把这些描述符的标题替换为一个在每次绘制时读取当前 locale 的函数，因此切换语言无需重新注册即可生效，卸载时又会把原值放回去。[src/tab-titles.ts](src/tab-titles.ts) 中没有列出的 locale 会回退到 tab 自带的标题，中文读者因此保留原有措辞。

该表覆盖导航栏注册的十四个 tab：`dsh-better-sidebar` 的六个内置 tab（它们与其他插件走同一个注册表），以及来自 `dsh-openviking`、`dsh-prototype`、`dsh-mds`、`dsh-ssh-tunnel`、`dsh-sidebar-qa`、`dsh-docs-panel` 与 `dsh-flowglass` 的 tab。一个测试把这些 id 钉住：构建期没有任何东西会察觉插件重命名了自己的 tab，因此这个钉子把静默回归变成一次失败的测试。

这是对其他包文案的覆盖，**完备性门禁并不覆盖它**：门禁把字典对照已附带面板所注册的 namespace 断言，而这些标签不属于任何 namespace。回退机制正是让过期条目不至于破坏标签的东西——它会显示 tab 自己的标题。

只有 tab 标签会被触及。这些 tab 打开的面板由各自的插件渲染，超出本包所能寻址的范围：`dsh-flowglass` 的内部尤其通篇是中文。

品牌名在所有语言中都保持不译——`dsh-flowglass` 没有发布拉丁字母名称，因此用其包名，而不是为「流镜」臆造一个译名。
