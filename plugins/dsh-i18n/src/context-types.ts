...
/** One language as a language pack adds it to the selectable set. */
export interface LocaleDefinition {
  /** BCP 47-style locale id: what `setLocale` takes and the settings document stores. */
  id: string
  /** Display name in its own language — a language names itself. */
  label: string
  /** Registered locale consulted when this language lacks a dictionary key; the chain ends at `en`. */
  fallback: string
}
...
export interface LocaleService {
  ...
  /**
   * Add a language to the selectable set.
   * @param definition - the locale id, its display name, and its fallback locale.
   * @returns disposer removing it (the reader falls back off it).
   * @throws Error when the id is already registered or the fallback is unknown.
   */
  addLanguage(definition: LocaleDefinition): () => void
  /**
