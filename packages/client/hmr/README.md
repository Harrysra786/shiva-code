...
Run a tsdown watch process that writes the plugin's `lib/client.js` against the same host; rebuilt plugins are then swapped into the running browser automatically, one at a time.
